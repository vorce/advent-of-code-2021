...
// initial idea... turned out to be wrong :)
pub fn highestFrequency(start: []const i32) !i32 {
    var crab_dist = std.AutoHashMap(i32, i32).init(std.heap.page_allocator);
    defer crab_dist.deinit();

    var i: usize = 0;
    while (i < start.len) : (i += 1) {
        if (crab_dist.get(start[i])) |nr| {
            try crab_dist.put(start[i], nr + 1);
        } else {
            try crab_dist.put(start[i], 1);
        }
    }

    var top_frequency: i32 = 0;
    var top_value: i32 = 0;
    var distribution_iter = crab_dist.iterator();
    while (distribution_iter.next()) |entry| {
        if (entry.value_ptr.* >= top_frequency) {
            top_frequency = entry.value_ptr.*;
            top_value = entry.key_ptr.*;
        }
    }
    return top_value;
}
...
test "highestFrequency" {
    const input = [_]i32{ 16, 1, 2, 0, 4, 2, 7, 1, 2, 14 };
    const expected_result: i32 = 2;

    const result: i32 = try highestFrequency(input[0..]);

    try testing.expectEqual(expected_result, result);
}

pub fn medianNr(positions: []const i32) !i32 {
    var buf = try allocator.alloc(i32, positions.len);
    std.mem.copy(i32, buf, positions[0..]);
    std.sort.sort(i32, buf, {}, comptime std.sort.asc(i32));
...
test "medianNr" {
    ...
    const result: i32 = try medianNr(input[0..]);

    try testing.expectEqual(expected_result, result);
}

pub fn costOfMove(positions: []const i32, target: i32) !i32 {
    var cost: i32 = 0;
    for (positions) |pos| {
        const moves = try std.math.absInt(pos - target);
        cost += moves;
    }
    return cost;
}
