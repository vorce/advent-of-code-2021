...
pub fn averageNr(positions: []const i32) i32 {
    var sum: i32 = 0;
    for (positions) |pos| {
        sum += pos;
    }
    return @divFloor(sum, @intCast(i32, positions.len));
}

pub fn costOfMove2(positions: []const i32, target: i32) !i32 {
    var cost: i32 = 0;
    for (positions) |pos| {
        const moves = try std.math.absInt(pos - target);
        var move_cost: i32 = 0;
...
test "costOfMove2" {
    const input = [_]i32{ 16, 1, 2, 0, 4, 2, 7, 1, 2, 14 };
    const expected_result: i32 = 168;
    const result = try costOfMove2(input[0..], 5);

    try testing.expectEqual(expected_result, result);
}

pub fn part2(start: []const i32) !i32 {
    const target = averageNr(start[0..]);
    std.debug.print("Part 2 target: {d}\n", .{target});
    const cost = try costOfMove2(start, target);
    return cost;
}
...
pub fn main() !void {
    ...
    const result_part2: i32 = try part2(input[0..]);
    std.debug.print("Part 2 cost: {d}\n", .{result_part2});
}
